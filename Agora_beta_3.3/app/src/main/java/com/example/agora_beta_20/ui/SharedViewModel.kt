...
import com.example.agora_beta_20.data.api.ScheduleRequestAdd
...
class SharedViewModel : ViewModel() {
     ...
     * 일정 추가
     */
    fun addTask(scheduleRequest: ScheduleRequestAdd) {
        viewModelScope.launch {
            try {
    ...
    fun updateTaskCompletionStatus(id: Long, isCompleted: Boolean) {
        viewModelScope.launch {
            try {
                // 서버 요청
                val updatedTaskResponse = repository.updateSchedule(
                    id,
                    ScheduleRequest(
                        email = "user@example.com", // 사용자의 이메일
                        title = "", // 필요한 경우 Title 제공
                        category = "", // 필요한 경우 Category 제공
                        schedule = "", // 필요한 경우 Schedule 제공
                        reminderDate = "", // 필요한 경우 Reminder Date 제공
                        completionStatus = isCompleted // 업데이트된 상태
                    )
                )
                ...
                Log.e("SharedViewModel", "Failed to update completion status: ${e.message}")
            }
        }
    }
